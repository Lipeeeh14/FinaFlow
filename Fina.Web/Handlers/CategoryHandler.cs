...
public class CategoryHandler(IHttpClientFactory httpClientFactory) 
    : ICategoryHandler
{
    ...
    public async Task<PagedResponse<IEnumerable<Category>?>> GetAllAsync(GetAllCategoriesRequest request)
        => await _client.GetFromJsonAsync<PagedResponse<IEnumerable<Category>?>>($"v1/categories")
        ?? new PagedResponse<IEnumerable<Category>?>(null, 400, "Não foi possível obter as categorias.");
